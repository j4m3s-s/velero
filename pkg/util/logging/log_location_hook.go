...
const (
	...
	veleroPackage           = "github.com/j4m3s-s/velero/"
...
func (h *LogLocationHook) Fire(entry *logrus.Entry) error {
	...
	// skip 2 frames:
	//   runtime.Callers
	//   github.com/j4m3s-s/velero/pkg/util/logging/(*LogLocationHook).Fire
...
func removeVeleroPackagePrefix(file string) string {
	if index := strings.Index(file, veleroPackage); index != -1 {
		// strip off .../github.com/j4m3s-s/velero/ so we just have pkg/...
