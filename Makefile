...
shell: build-dirs build-env
	@# bind-mount the Velero root dir in at /github.com/j4m3s-s/velero
	...
	@docker run \
		...
		--rm \
		-u $$(id -u):$$(id -g) \
		-v "$$(pwd):/github.com/j4m3s-s/velero:delegated" \
...
#		make release
#
# To run the release, which will publish a *DRAFT* GitHub release in github.com/j4m3s-s/velero 
